<x-tools.shell :tool="$tool" :catalog="$catalog">
    <x-tools.workspace input-label="Filter Pencarian MIME Type"
                      input-hint="ketik ekstensi berkas (misal: png, pdf) atau tipe (misal: image, application)"
                      output-label="Tabel Referensi MIME Types"
                      placeholder="contoh: json, webp, pdf, video">
        <x-slot:actions>
            <x-ui.button data-action="search" variant="primary">Filter MIME</x-ui.button>
        ...
        <x-slot:options>
            <label class="flex items-center gap-2">
                ...
                <select data-option="format" class="border-3 border-ink bg-white px-2 py-1.5 text-sm font-bold">
                    ...
                    <option value="nginx">Blok Nginx (types { ... })</option>
                </select>
            </label>
        </x-slot:options>
    </x-tools.workspace>
